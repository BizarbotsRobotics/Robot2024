...
class FourNoteLeftSide(commands2.SequentialCommandGroup):

    def __init__(self, drive: SwerveDrive, shoober: Shoober, intake: Intake, conveyor: Conveyor):
        super().__init__(
        )
        self.addCommands(ThreeNoteLeftAutoCmd(drive, shoober, intake, conveyor),IntakeToShooterCmd(intake, conveyor, shoober).deadlineWith(
                                RunPathCmd(drive, "FourNoteLeftSide"),
                                ), 
                                RunPathCmd(drive, "FourNoteLeftSideScore"),
